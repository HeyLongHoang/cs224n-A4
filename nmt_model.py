...
class NMT(nn.Module):
    ...
    def encode(self, source_padded: torch.Tensor, source_lengths: List[int]) -> Tuple[torch.Tensor, Tuple[torch.Tensor, torch.Tensor]]:
        ...
        init_decoder_hidden = self.h_projection(last_hidden.permute(1,0,2).reshape(-1, 2 * last_hidden.size(2)))
        init_decoder_cell = self.c_projection(last_cell.permute(1,0,2).reshape(-1, 2 * last_cell.size(2)))
    ...
    def decode(self, enc_hiddens: torch.Tensor, enc_masks: torch.Tensor,
        ...
        for Y_t in torch.split(Y, 1): # Y_t of shape (1, b, e)
            Y_t = Y_t.squeeze(0)
    ...
    def step(self, Ybar_t: torch.Tensor,
        ...
        # Set e_t to -inf where enc_masks has 1 ~ padded values
